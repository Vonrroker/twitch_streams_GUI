#!/home/dayham/Desktop/Projetos/streamlink_twitch_GUI_Kivy/venv/bin/python3
...
info: https://dev.twitch.tv/docs/authentication/#getting-tokens """
...
class BoxMain(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.hand_area = [
            [self.ids.btn_atualizar.size, self.ids.btn_atualizar.pos],
    ...
    def set_cursor(self, *args):
        ...
        pos_y = args[1][1]
        is_area = [
            self.hand_area[i][1][0]
            < pos_x
            < (self.hand_area[i][0][0] + self.hand_area[i][1][0])
            and (self.hand_area[i][0][1] + self.hand_area[i][1][1])
            > pos_y
            > self.hand_area[i][1][1]
    ...
    def play(self, go: str, qlt="best"):
        ...
        if not self.ids.chkauto.active and qlt == "best":
            UrlRequest(
                url="https://api.twitch.tv/kraken/videos/followed?limit=90&sort=views",
                req_headers={
                    "Accept": "application/vnd.twitchtv.v5+json",
                    "Client-ID": cred["client_id"],
                    "Authorization": f'OAuth {cred["oauth_token"]}',
    ...
    def extrair_resol(self, *args, go):
        ...
        list_resol = next(
            (
                x
                for x in args[0][1]["videos"]
                if x["channel"]["name"] == go.lower()
                # if x["status"] == "recording" and x["channel"]["name"] == go.lower()
            ),
            False,
        )
...
class PopUpProgress(Popup):
    def __init__(self, chk_vlc=False, **kwargs):
        ...
        proc = [x.info["name"] for x in psutil.process_iter(["name"])]
...
class BoxImg(BoxLayout):
    def __init__(self, text, **kwargs):
        ...
        self.ids.lbl.text = (
            "{} - {} - {:,}[ref={}][color=ff0000] ... [/color][/ref]".format(
                text[0].capitalize(), text[1], text[2], text[0]
            ).replace(",", ".")
        )
        Window.bind(on_resize=self.resize)
